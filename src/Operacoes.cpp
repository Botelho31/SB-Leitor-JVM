...
void Operacoes::ldc()
{
	...
	cp_info cp = f->cp->constant_pool[index];
	if (cp.tag == STRING)
	{
		f->operandos->push((int *)(f->cp->constant_pool[cp.info[0].U2].info[1].array));
	}
	else
	{
		element aux;
...
void Operacoes::ldc2_w()
{
	...
	if (f->cp->constant_pool[index].tag == LONG)
	{
		valPushLong = Utils::u4ToLong(f->cp->constant_pool[index].info[0], f->cp->constant_pool[index + 1].info[0]);
		f->operandos->push(int64_t(valPushLong));
	}
	else
	{
		valPushDouble = Utils::u4ToDouble(f->cp->constant_pool[index].info[0], f->cp->constant_pool[index + 1].info[0]);
...
void Operacoes::lload()
{
	...
	typedElement aux = f->locals->get(int(index));
	f->operandos->push(int64_t(aux.value.l));
}
...
void Operacoes::iaload()
{
	...
	Array *arrayRef = (Array*)arrayElement.value.pi;
	if (arrayRef == nullptr)
		throw std::runtime_error("NullPointerException");
	f->operandos->push(*arrayRef->data[index]);
}
...
void Operacoes::iastore()
{
	...
	if (arrayRef == nullptr)
		throw std::runtime_error("NullPointerException");
	if (valor.type != arrayRef->data[0]->type)
...
void Operacoes::frem()
{
	...
	if (Utils::checkFloat(value1.f) == 3 || Utils::checkFloat(value2.f) == 3)
	{
		result.value.i = Float_NaN;
	}
	else if (Utils::checkFloat(value1.f) == 1 || Utils::checkFloat(value1.f) == 2 || value2.f == 0.0)
	{
		result.value.i = Float_NaN;
	}
	else if (Utils::checkFloat(value1.f) == 0 && (Utils::checkFloat(value2.f) == 1 || Utils::checkFloat(value2.f) == 2))
	{
		result.value.f = value1.f;
	}
	else if (value1.f == 0.0 && Utils::checkFloat(value2.f) == 0)
	{
		result.value.f = 0.0;
	}
	else
...
void Operacoes::drem()
{
	...
	if (Utils::checkDouble(value1.d) == 3 || Utils::checkDouble(value2.d) == 3)
	{
		result.value.l = Double_NaN;
	}
	else if (Utils::checkDouble(value1.d) == 1 || Utils::checkDouble(value1.d) == 2 || value2.d == double(0.0))
	{
		result.value.l = Double_NaN;
	}
	else if (Utils::checkDouble(value1.d) == 0 && (Utils::checkDouble(value2.d) == 1 || Utils::checkDouble(value2.d) == 2))
	{
		result.value.d = value1.d;
	}
	else if (value1.d == 0.0 && Utils::checkDouble(value2.d) == 0)
	{
		result.value.d = 0.0;
	}
	else
...
void Operacoes::fcmpl()
{
	...
	res1 = Utils::checkFloat(value1);
	res2 = Utils::checkFloat(value2);
...
void Operacoes::dcmpl()
{
	...
	res1 = Utils::checkDouble(value1);
	res2 = Utils::checkDouble(value2);
	//se value1 ou value2 for NaN entao adiciona 1 na pilha de operandos
	if (res1 == 3 || res2 == 3)
	{
		f->operandos->push(int(1));
...
void Operacoes::tableswitch()
{
	...
	uint8_t mod = (f->pc - f->m->attributes->attributes[0]->info->code.code) % 4;
	f->pc += mod;
...
void Operacoes::lookupswitch()
{
	...
	int diff = (f->pc - f->m->attributes->attributes[0]->info->code.code) % 4;
...
{
	int64_t value = f->operandos->pop().l;
...
ClasseEstatica *Operacoes::getStaticClassThatHasField(ClasseEstatica *base, std::string field_name)
{
	...
	ClasseEstatica *child = MethodArea::getClass(base->getDef()->getCP()->dereferenceIndex(cp_index));
...
void Operacoes::getstatic()
{
	...
	cp_info name_and_type_element = f->cp->constant_pool[cp_element.info[1].U2];
	...
	std::string name = f->cp->dereferenceIndex(name_and_type_element.info[0].U2);
	std::string descriptor = f->cp->dereferenceIndex(name_and_type_element.info[1].U2);
	...
	if (static_class == NULL)
		throw std::runtime_error("Field nao existe na classe definida!");
...
void Operacoes::getfield()
{
	...
	int index = f->cp->constant_pool[indexbyte].info[1].U2;
	index = f->cp->constant_pool[index].info[0].U2;
	typedElement ret = ci->getField(f->cp->dereferenceIndex(index));
...
void Operacoes::putfield()
{
	...
	if (f->cp->dereferenceIndex(f->m->name_index) == "<init>")
	{
		ci->setFinals(f->cp->dereferenceIndex(index), value);
	}
	else
	{
		ci->setField(f->cp->dereferenceIndex(index), value);
	}
}
...
void Operacoes::invokevirtual()
{
	...
	if (class_name.find("java/") != std::string::npos)
	{
		...
		if (class_name == "java/io/PrintStream" && (name == "print" || name == "println"))
		{
			if (descriptor != "()V")
			{
				...
				case RT_LONG:
					...
					break;
				case RT_REFERENCE:
					std::cout << Utils::showUTF8((unsigned char *)element.value.pi, 0);
					break;
				case RT_BOOL:
		...
		else if (class_name == "java/lang/String" && name == "length")
		{
			...
			if (element.realType == RT_REFERENCE)
			{
				...
				ret.value.i = Utils::showUTF8((unsigned char *)element.value.pi, 0).size();
		...
		else if (class_name == "java/lang/String" && name == "equals")
		{
			...
			if (element_1.realType == RT_REFERENCE && element_2.realType == RT_REFERENCE)
			{
				...
				if (Utils::showUTF8((unsigned char *)element_1.value.pi, 0) == Utils::showUTF8((unsigned char *)element_2.value.pi, 0))
				{
					ret.value.b = 1;
				}
				else
	...
	else
	{
		...
		fs->addFrame(
			...
			instance->getStatic()->getDef()->getClassThatHasSearchedMethod(name, descriptor)->getCP()
            );
...
void Operacoes::invokespecial()
{
	...
	std::string classe = f->cp->dereferenceIndex(cp_element.info[0].U2);

	cp_info metodo = f->cp->constant_pool[cp_element.info[1].U2];
	if (metodo.tag != NAME_AND_TYPE)
	{
		throw std::runtime_error("Elemento da constant pool apontado por index, não é uma referencia para NAME_AND_TYPE!");
	}

	std::string name = f->cp->dereferenceIndex(metodo.info[0].U2);
	std::string desc = f->cp->dereferenceIndex(metodo.info[1].U2);
	...
	if (classe.find("java/") != std::string::npos)
	{
		std::cerr << "ERRO: \"" << name << "\" nao definido." << std::endl;
		exit(1);
	}
	else
	{
		...
		std::vector<typedElement> parametros;
		for (int i = 0; i < count; i++)
		{
			typedElement prox = f->operandos->popTyped();
			parametros.push_back(prox);
		}

		typedElement object_element = f->operandos->popTyped();

        parametros.push_back(object_element);
		...
		fs->addFrame(
			...
			instance->getStatic()->getDef()->getClassThatHasSearchedMethod(name, desc)->getCP());
		//adiciona os parâmetros ao vetor de variáveis locais
		
		std::reverse(parametros.begin(),parametros.end());
...
void Operacoes::invokestatic()
{
	...
	if (class_name.find("java/") != std::string::npos)
	{
		std::cerr << "Tentando invocar metodo estatico invalido: " << name << std::endl;
	}
	else
	{
		...
		fs->addFrame(
			...
			ce->getDef()->getClassThatHasSearchedMethod(name, descriptor)->getCP());
...
{
	uint16_t indexbyte = getNBytesValue(2, &f->pc);
	std::string classe = f->cp->dereferenceIndex(indexbyte);
...
void Operacoes::newarray()
{
	...
	int32_t index = f->operandos->pop().i;
...
void Operacoes::arraylength()
{
	...
	arrayRef = (Array *)f->operandos->pop().pi;
	if (arrayRef == nullptr)
		throw std::runtime_error("Null pointer");

	int length = arrayRef->data.size();
	f->operandos->push(length);
}
...
void Operacoes::multianewarray()
{
	...
	std::string multiArrayType = class_name.substr(count + 1, class_name.size() - count - 2); // em caso de ser uma referência (e.g. [[[Ljava/lang/String;)
	...
	element.value.pi = (int*)arrayValue;
	element.realType = RT_REFERENCE;
	element.type = TYPE_REFERENCE;
...
Array *Operacoes::buildMultiArray(int index,uint8_t type,uint8_t realType, std::vector<int> dimension_array)
{
    ...
    for (int i = 0; i < dimension_array[index]; i++)
    {
        ...
        array_value->value.pi = (int*)buildMultiArray(index - 1, type,realType, dimension_array);
		array_value->type = TYPE_REFERENCE;
		array_value->realType = RT_REFERENCE;
